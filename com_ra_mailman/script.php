...
//
        $this->checkColumn('ra_mail_lists', 'group_primary', 'A', "VARCHAR(4) DEFAULT NULL AFTER group_code; ");
        $this->checkColumn('ra_mail_lists', 'chat_list', 'A', "INT NOT NULL DEFAULT 0 AFTER group_primary; ");
        $this->checkColumn('ra_mail_shots', 'author_id', 'D');
        // ALTER TABLE `j4_ra_mail_subscriptions` ADD `reminder_sent` DATETIME NULL AFTER `expiry_date`;
        $this->checkColumn('ra_mail_subscriptions', 'reminder_sent', 'A', "DATETIME NULL AFTER `expiry_date`; ");
        return true;
    }
